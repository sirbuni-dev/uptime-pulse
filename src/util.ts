...
export const appName = "Uptime Pulse";
export const DOWN = 0;
export const UP = 1;
...
/**
 * @deprecated Use log.debug (https://github.com/louislam/uptime-pulse/pull/910)
...
class Logger {
    /**
     * UPTIME_PULSE_HIDE_LOG=debug_monitor,info_monitor
     *
     * Example:
     *  [
     ...
     */
    constructor() {
        if (typeof process !== "undefined" && process.env.UPTIME_PULSE_HIDE_LOG) {
            const list = process.env.UPTIME_PULSE_HIDE_LOG.split(",").map((v) => v.toLowerCase());
            ...
            this.debug("server", "UPTIME_PULSE_HIDE_LOG is set");
    ...
    log(module: string, level: "INFO" | "WARN" | "ERROR" | "DEBUG", ...msg: unknown[]) {
        ...
        if (process.env.UPTIME_PULSE_LOG_FORMAT === "json") {
            const msgString = msg
                .map((m) => {
                    if (typeof m === "string") {
                        return m;
